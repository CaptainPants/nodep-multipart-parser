...
import {
    percentEncodeCharactersUsingTextEncoder,
    slowCompatiblePercentEncodeCharactersBlobConstructor,
    writeExtendedValue,
} from "./writeExtendedValue.js";
...
describe("writeExtendedValue", () => {
    test("test", async () => {
        ...
        expect(await writeExtendedValue("£ and € rates")).toEqual(
...
describe("percentEncodeCharacters", () => {
    const inputs = [
        ...
        [
            "£, €, abcdefghijk, abcdefghijk, xyz",
            "%C2%A3%2C%20%E2%82%AC%2C%20%61%62%63%64%65%66%67%68%69%6A%6B%2C%20%61%62%63%64%65%66%67%68%69%6A%6B%2C%20%78%79%7A",
        ],
    ];
    ...
    test.each(inputs)(
        ...
        async (input, expected) => {
            const actual =
                await slowCompatiblePercentEncodeCharactersBlobConstructor(
                    input
                );

...
export class HttpClient {
    async request(request: HttpRequest): Promise<HttpResponse> {
        ...
        const data = await this.#prepareData(request.content, request.responseType);

        await this.#wrapInPromise(
            xhr,
            () => {
    ...
    /**
      ...
      * resolves if the request ended properly, rejects with a CancelledError
      * if it was aborted.
      */
    #wrapInPromise(
        xhr: XMLHttpRequest,
    ...
    async #prepareData(
        ...
        if (type === 'text') {
            ...
            if (content.data.source instanceof Blob || isArrayBuffer(content.data.source)) {

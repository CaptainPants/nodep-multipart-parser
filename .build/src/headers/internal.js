...
function readToNextLine(state) {
  ...
  if (end === void 0) {
    throw new import__.ParseError("Unexpected.");
  }
...
function readToken(state) {
  ...
  if (!token) {
    if (isFinished(state)) {
      throw new import__.ParseError("Unexpected EOF, expected token.");
    }
    throw new import__.ParseError(`Unexpected '${state.string[state.index]}', expected token.`);
  }
  return token;
}
...
function readQuoted(state) {
  if (state.string[state.index] != '"') {
    throw new import__.ParseError(`Unexpected ${state.string[state.index]}, expected ".`);
  }
  ++state.index;
  const res = [];
  for (; ; ) {
  ...
  if (state.index >= state.end) {
    throw new import__.ParseError(`Unexpected EOF, expecting '"'.`);
  }
  const closeQuote = state.string[state.index];
  if (closeQuote != '"') {
    throw new import__.ParseError(`Unexpected '${closeQuote}', expecting '"'.`);
  }
  ++state.index;
...
function readOneParameter(state) {
  ...
  if (!parameterName) {
    if (isFinished(state)) {
      ...
      throw new import__.ParseError(`Unexpected ${state.string[state.index]}, expecting a token.`);
    }
  }
  consumeOptionalWhitespace(state);
  if (isFinished(state) || state.string[state.index] !== "=") {
    throw new import__.ParseError(`Unexpected ${state.string[state.index]}, expecting an equals sign.`);
  }
  ++state.index;
  consumeOptionalWhitespace(state);
  if (isFinished(state)) {
    throw new import__.ParseError(`Unexpected EOF, expecting a token or quoted-string.`);
  }
  let value;
  if (state.string[state.index] === '"') {
...
function processParametersIfPresent(state) {
  ...
  for (; ; ) {
    ...
    if (semicolon !== ";") {
      throw new import__.ParseError(`Unexpected '${semicolon}' when expecting a semi-colon ';'.`);
    }
    ++state.index;
    const parameter = readOneParameter(state);
    if (!parameter) {
      if (!isFinished(state)) {
        throw new import__.ParseError(`Unexpected '${state.string[state.index]}' when expecting parameter or EOF.`);
      }
      break;
    }
    res.push(parameter);
